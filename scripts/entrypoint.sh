...
# Install openssl CLI if not present
if ! command -v openssl >/dev/null 2>&1; then
  echo "[entrypoint] Installing openssl..."
  apk add --no-cache openssl >/dev/null 2>&1 || echo "[entrypoint] WARNING: could not install openssl"
fi
...
if [ ! -f "$CERT_DIR/fullchain.pem" ]; then
  echo "[entrypoint] No LE cert at $CERT_DIR — attempting self-signed bootstrap..."
  mkdir -p "$CERT_DIR" 2>&1 && \
    openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
      -keyout "$CERT_DIR/privkey.pem" \
      -out "$CERT_DIR/fullchain.pem" \
      -subj "/CN=$DOMAIN" && \
    echo "[entrypoint] Self-signed cert created at $CERT_DIR" \
    || echo "[entrypoint] WARNING: cert generation failed — check permissions on ./certbot/conf/ on the host"
else
